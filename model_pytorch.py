...
class Block(nn.Module):
    ...
    def forward(self, x):
        a = self.attn(x)  # Tensor [4,257,768]
        n = self.ln_1(x + a)  # Tensor [4,257,768]
        m = self.mlp(n)  # Tensor [4,257,768]
        h = self.ln_2(n + m)  # Tensor [4,257,768]
        return h
...
class LMHead(nn.Module):
    ...
    def forward(self, h):
        ...
        # h_trunc： torch.float32，torch.Size([1024, 768])
...
def load_openai_pretrained_model(model, n_ctx=-1, n_special=-1, n_transfer=12, n_embd=768, path='./model/',
    ...
    names = json.load(open(
        path_names + 'parameters_names.json'))  # length:147  list: ['model/we:0','model/h0/attn/c_attn/w:0','model/h0/attn/c_attn/b:0',...,'model/clf/w:0','model/clf/b:0']
    shapes = json.load(
        open(path + 'params_shapes.json'))  # length:146  <class 'list'>: [[512, 768], [40478, 768], [1, 768, 2304],...]
    offsets = np.cumsum([np.prod(shape) for shape in shapes])
    init_params = [np.load(path + 'params_{}.npy'.format(n)) for n in range(10)]
    init_params = np.split(np.concatenate(init_params, 0), offsets)[
                  :-1]  # len:146 <class 'list'>: [ndarray(size=393216=512*768),ndarray,]
    init_params = [param.reshape(shape) for param, shape in zip(init_params, shapes)]
    if n_ctx > 0:  # 257
        init_params[0] = init_params[0][:n_ctx]  # [257,768]
    if n_special > 0:
    ...
    if n_transfer == -1:
        ...
        # layers to be transferred, 1 means the 1st embedding layer, 12 heads * 12 layers each
    ...
    # names has 147 elements with last two clf as param names
    #
