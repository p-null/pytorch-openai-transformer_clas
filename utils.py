...
def encode_dataset(*splits, encoder):
    encoded_splits = []
    for split in splits:
        # splits: tuple: size 3, ((trX1, trY), (vaX1, vaY), (teX1, ))
        # split: tuple: size 2, (trX1, trY) or size 1, (teX1, )
        fields = []
        # pdb.set_trace()
        for field in split:
            # split: (['string', 'string', ...], ndarray)
            # field: list or ndarray
            # check if the first element is string, if so, field is text field , otherwise is label field
